/**
 * <Card>…</Card>
 * <Card padding="none" shadow="lg">…</Card>
 */
export default function Card({ children, className = '', padding = 'md', shadow = 'sm', onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`bg-white rounded-2xl border border-slate-100 ${paddings[padding]} ${shadows[shadow]} ${onClick ? 'cursor-pointer hover:shadow-md transition-shadow' : ''} ${className}`}
    >
      {children}
    </div>
  )
